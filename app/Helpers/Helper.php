    ...
    function send_email($user, $type, $shortcodes = [])
    {
        try {
            ...
            if ($status == 1) {
                ...
                if ($config->id == 1) {
                    send_php_mail($to_email, $user->username, $config->from_email, $email_template->subj, $message);
                } else if ($config->id == 2) {
                    send_smtp_mail($config, $to_email, $user->username, $config->from_email, $general->site_title, $email_template->subj, $message);
                } else if ($config->id == 3) {
                    sendMailBySendGrid($config, $to_email, $user->username, $config->from_email, $general->site_title, $email_template->subj, $message);
                }
                return true;
            }
            return false;
...
if (!function_exists('getSetting2')) {
    function getSetting2()
    {
        $settingFile = 'system_general_setting.json';
        try {
            if (!file_exists(base_path($settingFile))) {
                $gs = \Modules\Setting\Model\GeneralSetting::first();
                Storage::disk('localRoot')->put($settingFile, json_encode($gs));
            }
            $setting = file_get_contents(base_path($settingFile));
...
function send_general_email($email, $subject, $message, $receiver_name = '')
{
    ...
    return send_php_mail($email, $receiver_name, $general->sendEmail, $subject, $message);
}
...
if (!function_exists('theme')) {
    ...
    {
        return 'frontend.' . Settings('frontend_active_theme') . '.' . $fileName;
...
if (!function_exists('courseSetting')) {
    ...
    {
        return $data = CourseSetting::first();
    }
}
        ...
        try {
            ...
            if ($value == "frontend_active_theme") {
                ...
                return Cache::rememberForever('frontend_active_theme', function () {
                    $setting = GeneralSetting::first();
                    return $setting->frontend_active_theme;
                });
