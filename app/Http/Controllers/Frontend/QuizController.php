    ...
    public function quizSubmit(Request $request)
    {
        ...
        try {
            ...
            if ($request->from == "course") {
                $check = Lesson::where('course_id', $courseId)->where('quiz_id', $quizId)->first();
                if ($check) {
                    $lesson = LessonComplete::where('course_id', $courseId)->where('lesson_id', $check->id)->where('user_id', Auth::id())->first();
                    if (!$lesson) {
