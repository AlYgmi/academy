...
class InstructorController extends Controller
{
    ...
    public function instructorDetails($id, $name, Request $request)
    {
        try {
            ...
            $InstructorSetup = InstructorSetup::first();
            $courses = Course::where('user_id', $id)->with('enrollUsers', 'lessons')->where('status', 1)->orderBy('total_rating', 'desc')->paginate(12);
            ...
            if (isModuleActive('BundleSubscription')) {
                ...
                $BundleCourse = $bundleCourse->all($id);
            } else {
                $BundleCourse = null;
            }
