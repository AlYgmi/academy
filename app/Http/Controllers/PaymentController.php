            ...
            if ($request->payment_method == "Sslcommerz") {
                ...
                try {
                    $response = $this->payPalGateway->purchase(array(
                        'amount' => convertCurrency(Settings('currency_code') ?? 'BDT', 'USD', $checkout_info->purchase_price),
                        'currency' => 'USD',
                ...
                $amount = convertCurrency(Settings('currency_code') ?? 'USD', 'RON', $checkout_info->purchase_price);
...
//            $formData = array('number' => '4242424242424242', 'expiryMonth' => '6', 'expiryYear' => '2030', 'cvv' => '123');
    ...
    public static function payWithGateWay($response, $gateWayName, $user = null)
    {
        try {
            if (Auth::check()) {
                $user = Auth::user();
...
//                $total = Cart::where('user_id', $user->id)->sum('price');
                        ...
                        if ($cart->course_id != 0) {
                            ...
                            if ($cart->renew != 1) {
                                foreach ($bundleCheck->course as $course) {
                                    ...
                                    $enroll->purchase_price = $course->course->price;
                                    $enroll->coupon = null;
                            ...
                            $bundleCommission = BundleSetting::first();
                            ...
                            $notification = new Notification();
                            $notification->author_id = $bundleCheck->user_id;
                            $notification->user_id = $checkout_info->user->id;
                            $notification->course_id = 0;
                            $notification->course_enrolled_id = $enroll->id;
                            $notification->status = 0;
    ...
    public static function payWithGateWay2($response, $gateWayName)
    {
        try {
