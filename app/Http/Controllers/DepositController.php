...
//        }
        ...
        if ($data['method'] == "Sslcommerz") {
            ...
            $response = $this->payPalGateway->purchase(array(
                'amount' => convertCurrency(Settings('currency_code') ?? 'BDT', 'USD', $data['deposit_amount']),
                'currency' => 'USD',
            ...
            if ($response->isRedirect()) {
                ...
                if ($response->getCode() == "amount_too_small") {
                    $amount = number_format(convertCurrency('USD', strtoupper(Settings('currency_code') ?? 'BDT'), 0.5),2);
